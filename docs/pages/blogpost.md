...
<!--Let's re-write the paragraph above together, making sure to distinguish the NCA model and the conv-net used to compute style loss.
    * NCA model generates a texture by iteratively applying it's neural layer operations to an image grid
    * the final texture image is used as input to a pre-trained convolutional neural network layer (we used vgg16)
    * the target texture image is also used as input to the conv-net model
    * We then calculate Gram matrices for the activations, or features, of several interior layers of the conv-net
    * As you mention below, the loss is the average squared difference between the target and model output Gram matrices
 -->

<!---These equations will need to be rendered in sympy and then included as images here.--->
